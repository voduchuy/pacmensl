...
// propensity function for toggle
    int propensity(const int reaction, const int num_species, const int num_states, const PetscInt *X, double *outputs, void* args ) {
        ...
        switch ( reaction ) {
            case 0:
                ...
                break;
            case 1:
                for (int i{0}; i < num_states;++i){outputs[i] = 1.0 / ( 1.0 + ayx * pow( PetscReal( X_view[i][ 1 ] ), nyx ));}
                break;
            case 2:
            ...
            case 3:
                ...
                break;
            case 4:
                for (int i{0}; i < num_states; ++i){outputs[i] = 1.0 / ( 1.0 + axy * pow( PetscReal( X_view[i][ 0 ] ), nxy ));}
                break;
            case 5:
...
TEST_F( OdeTest, use_cvode_bdf ) {
    ...
    cvode_solver.set_print_intermediate( 0 );
...
TEST_F( OdeTest, use_krylov ) {
    ...
    krylov_solver.set_print_intermediate( 0 );
