...
PacmenslErrorCode pacmensl::SensFspMatrix<FspMatrixT>::GenerateValues(const pacmensl::StateSetBase &state_set,
  ...
  PacmenslErrorCode ierr;
  ierr = A_.GenerateValues(state_set, model.stoichiometry_matrix_, model.propensity_xfac_,
                           model.propensity_xfac_args_, model.propensity_tfac_, model.propensity_tfac_args_);
  PACMENSLCHKERRQ(ierr);
  ...
  for (int i{0}; i < num_parameters_; ++i) {
    ...
    ierr = dA_[i].GenerateValues(state_set,
                                ...
                                model.dpropensity_xfac_[i],
                                model.dpropensity_xfac_args_[i],
                                model.dpropensity_tfac_[i],
                                model.dpropensity_tfac_args_[i]);
    PACMENSLCHKERRQ(ierr);
  }
  return 0;
}
