...
int pacmensl::StationaryFspMatrixConstrained::GenerateValues(const pacmensl::StateSetBase &fsp,
                                                             const arma::Mat<int> &SM,
                                                             const pacmensl::PropFun &prop,
                                                             void *prop_args,
                                                             const pacmensl::TcoefFun &new_t_fun,
                                                             void *t_fun_args) {
  int ierr{0};
  ...
  ierr = new_t_fun(0.0, num_reactions_, &time_coefficients_[0], t_fun_args_);
  PACMENSLCHKERRQ(ierr);
  ierr = VecCreate(comm_, &diagonal_);
  CHKERRQ(ierr);
  ierr = VecSetSizes(diagonal_, get_num_rows_local(), PETSC_DECIDE);
  CHKERRQ(ierr);
  ierr = VecSetUp(diagonal_);
  CHKERRQ(ierr);
  ierr = VecSet(diagonal_, 0.0);
  CHKERRQ(ierr);
  PetscReal *d_array;
  ierr = VecGetArray(diagonal_, &d_array);
  CHKERRQ(ierr);
  int num_states = fsp.GetNumLocalStates();
  std::vector<PetscReal> d_vals(num_states);
  for (int i{0}; i < num_reactions_; ++i) {
    ierr = prop(i, state_list.n_rows, num_states, &state_list[0], &d_vals[0], nullptr);
    PACMENSLCHKERRQ(ierr);
    for (int j = 0; j < num_states; ++j) {
      d_array[j] += time_coefficients_(i) * d_vals[j];
    }
  }
  ierr = VecRestoreArray(diagonal_, &d_array);
...
int pacmensl::StationaryFspMatrixConstrained::Destroy() {
  int ierr;
  ierr = VecDestroy(&diagonal_);
  CHKERRQ(ierr);
  ierr = FspMatrixBase::Destroy();
  PACMENSLCHKERRQ(ierr);
  return 0;
}
...
int pacmensl::StationaryFspMatrixConstrained::Action(PetscReal t, Vec x, Vec y) {
  ...
  CHKERRQ(ierr);
  PetscReal sink_sum;
  ierr = VecSum(sink_entries_, &sink_sum);
  CHKERRQ(ierr);
  PetscReal* ylocal;
  ierr = VecGetArray(y, &ylocal);
  CHKERRQ(ierr);
  ierr = MPI_Reduce((void*) &sink_sum, (void*) &ylocal[0], 1, MPIU_REAL, MPIU_SUM, 0, comm_);
  CHKERRMPI(ierr);
  ierr = VecRestoreArray(y, &ylocal);
  CHKERRQ(ierr);
  return 0;
}
...
int pacmensl::StationaryFspMatrixConstrained::EvaluateOutflows(Vec sfsp_solution, arma::Row<PetscReal> &sinks) {
  int ierr;
  ierr = t_fun_(0.0, num_reactions_, time_coefficients_.memptr(), t_fun_args_);
  PACMENSLCHKERRQ(ierr);
  ierr = VecGetLocalVector(sfsp_solution, xx);
  CHKERRQ(ierr);
  ierr = VecSet(sink_entries_, 0.0);
  CHKERRQ(ierr);
  for (int i = 0; i < num_reactions_; ++i) {
  ...
  ierr = VecRestoreLocalVector(sfsp_solution, xx);
  CHKERRQ(ierr);
  sinks.resize(num_constraints_);
  const PetscReal *local_sinks;
  ierr = VecGetArrayRead(sink_entries_, &local_sinks);
  CHKERRQ(ierr);
  ierr = MPI_Allreduce((void*)local_sinks, (void*)sinks.memptr(), num_constraints_, MPIU_REAL, MPIU_SUM, comm_);
  CHKERRQ(ierr);
  return 0;
}
