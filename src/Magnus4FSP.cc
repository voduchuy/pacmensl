...
namespace cme {
    namespace petsc {
        ...
        void Magnus4FSP::step() {
            ...
            t_step = std::min(t_step_new, t_step_max);
        }
        ...
        void Magnus4FSP::solve() {
            ...
            while (t_now < t_final) {
                ...
                get_sinks();
                Real sink_sum = arma::sum(sinks);
                std::cout << "Sinks = " << sinks << std::endl;

                if (sink_sum >= t_now * fsp_tol / t_final) {
                    /* Set solution to the previous time point */
                    VecCopy(solution_old, solution_now);
                    t_now = t_old;
                    t_step_max = 0.5e0 * t_step;

                    /* Determine which sink states need expansion */
                    to_expand.fill(0);
                    arma::uvec indx = arma::sort_index(sinks, "descend");
                    Real tmp;
                    Int k = 0;
                    tmp = sinks(indx(k));
                    while (tmp < t_now * fsp_tol / t_final) {
                        ++k;
                        tmp += sinks(indx(k));
                        to_expand(indx(k)) = 1;
                    }
