...
* @param new_t_fun callable object for evaluating the time coefficients. See also TcoefFun.
* @param t_fun_args pointer to additional data for time function.
* @return error code, 0 if successful.
*/
PacmenslErrorCode FspMatrixConstrained::GenerateValues(const StateSetBase &state_set,
  ...
  ierr = FspMatrixBase::GenerateValues(state_set, SM, prop, prop_args, new_t_fun, t_fun_args);
  PACMENSLCHKERRQ(ierr);
  ...
  for (int i_reaction{0}; i_reaction < num_reactions_; ++i_reaction) {
    ...
    CHKERRQ(ierr);
    ierr = MatSetSizes(sinks_mat_[i_reaction], n_constraints, num_rows_local_, n_constraints, num_rows_local_);
    CHKERRQ(ierr);
...
FspMatrixConstrained::FspMatrixConstrained(const FspMatrixConstrained &A) : FspMatrixBase(A) {
  ...
  sinks_mat_.resize(num_reactions_);
  for (int i = 0; i < num_reactions_; ++i) {
    ierr = MatDuplicate(A.sinks_mat_[i], MAT_COPY_VALUES, &sinks_mat_[i]);
    PETSCCHKERRTHROW(ierr);
  }
}
